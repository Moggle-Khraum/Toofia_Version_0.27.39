...
func voiceOver():
	...
	if $"%infoGrapes".playing:
		$"%coverButton".show()


func _on_x_pressed():
	$"%infoGrapes".stream_paused = false
	print("Info Grapes Plays")
...
func _on_repeat_pressed():
	...
	if not $"%infoGrapes".playing or $"%askVoice5".playing or $"%choiceA".playing or $"%choiceB".playing or $"%choiceC".playing or $"%pickAnswer".playing:
		$"%coverButton".show()
		print("Cover Show")
	else:
		$"%coverButton".hide()
...
func _on_infoGrapes_finished():
	yield(get_tree().create_timer(0.9), "timeout")
	$'%askVoice5'.play()
	print("Play Grapes Info")

func _on_askVoice5_finished():
	yield(get_tree().create_timer(0.9), "timeout")
	$'%choiceA'.play()
	$'%A'.show()
	$"%A".disabled = true
...
func _on_retry_pressed() -> void:
	var retry = get_tree().change_scene("res://Levels/1PrutasAtGulay/Level5/level5.tscn")
	print('Retry level')
