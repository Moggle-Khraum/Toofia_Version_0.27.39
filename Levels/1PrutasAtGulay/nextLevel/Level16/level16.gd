...
func voiceOver():
	...
	if $'%infoRambutan'.playing:
		$"%coverButton".show()


func _on_x_pressed():
	$'%infoRambutan'.stream_paused = false
	$'%askVoice16'.stream_paused = false
...
func _on_pauseButton_pressed():
	...
	$'%infoRambutan'.stream_paused = true
	$'%askVoice16'.stream_paused = true
...
func _on_repeat_pressed():
	...
	if not $'%infoRambutan'.playing or $"%askVoice16".playing or $"%choiceA".playing or $"%choiceB".playing or $"%choiceC".playing or $"%pickAnswer".playing:
		$"%coverButton".show()
		print("Cover Show")
	else:
		$"%coverButton".hide()
