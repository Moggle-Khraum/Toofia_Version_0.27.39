...
func voiceOver():
	...
	$'%infoTurnip'.play()
	if $"%infoTurnip".playing:
		$"%coverButton".show()


func _on_x_pressed():
	$'%infoTurnip'.stream_paused = false
...
func _on_repeat_pressed():
	...
	if not $'%infoTurnip'.playing or $"%askVoice14".playing or $"%choiceA".playing or $"%choiceB".playing or $"%choiceC".playing or $"%pickAnswer".playing:
		$"%coverButton".show()
		print("Cover Show")
	else:
		$"%coverButton".hide()
