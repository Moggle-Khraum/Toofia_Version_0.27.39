...
func voiceOver():
	...
	if $"%infoTalong".playing:
		$"%coverButton".show()


func _on_x_pressed():
	$"%infoTalong".stream_paused = false
	print("Info Talong Plays")
...
func _on_repeat_pressed():
	...
	print("Emits Signal")
	$"%infoTalong".stop()
	print("Info Talong stop")
	$'%coverButton'.show()
	$"%A".hide()
	$"%B".hide()
	$"%C".hide()
	print("Hide Repeat Button")
	if not $"%infoTalong".playing or $"%askVoice7".playing or $"%choiceA".playing or $"%choiceB".playing or $"%choiceC".playing or $"%pickAnswer".playing:
		$"%coverButton".show()
		print("Cover Show")
	else:
		$"%coverButton".hide()
