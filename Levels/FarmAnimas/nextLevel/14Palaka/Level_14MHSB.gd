...
func _on_x_pressed() -> void:
	$"%infoFrog".stream_paused = true
	$'%frogVoice'.stream_paused = true
...
func _on_infoFrog_finished() -> void:
	yield(get_tree().create_timer(0.9), "timeout")
	$"%frogVoice".play()

func _on_frogVoice_finished():
	yield(get_tree().create_timer(0.9), "timeout")
	$'%AskVoice'.play()
	print('Play AskVoice')
...
func _on_nextQuestion_pressed() -> void:
	ScrollPop.play()
	yield(get_tree().create_timer(0.12), "timeout")
	var retry = get_tree().change_scene("res://Levels/FarmAnimas/nextLevel/15Unggoy/Level_15MHSB.tscn")
