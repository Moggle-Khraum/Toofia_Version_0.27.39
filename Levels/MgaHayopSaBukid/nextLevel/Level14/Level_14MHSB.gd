...
func _on_x_pressed():
	...
	$'%askVoice14'.stream_paused = false
	$'%patoVoice'.stream_paused = false
...
func _on_pauseButton_pressed():
	...
	$"%infoPato".stream_paused = true
	$'%askVoice14'.stream_paused = true
...
func _on_repeat_pressed():
	...
	if not $"%infoPato".playing or $'%patoVoice' or $"%askVoice14".playing or $"%choiceA".playing or $"%choiceB".playing or $"%choiceC".playing or $"%pickAnswer".playing:
		$"%coverButton".show()
		print("Cover Show")
	else:
		$"%coverButton".hide()
...
func _on_retry_pressed() -> void:
	ScrollPop.play()
	var retry = get_tree().change_scene("res://Levels/MgaHayopSaBukid/nextLevel/Level14/Level_14MHSB.tscn")
	print('Retry level')

func _on_nextQuestion_pressed() -> void:
	ScrollPop.play()
	yield(get_tree().create_timer(0.12), "timeout")
	var retry = get_tree().change_scene("res://Levels/MgaHayopSaBukid/nextLevel/Level15/Level_15MHSB.tscn")
